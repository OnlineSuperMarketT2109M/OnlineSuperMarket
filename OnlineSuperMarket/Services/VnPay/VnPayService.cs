...
namespace OnlineSuperMarket.Services.VnPay
{
    public class VnPayService
    {
        ...
        public string getPaymentUrl(Order order)
        {
            ...
            string amount = (order.total*100).ToString();
            string bankCode = config["VnPay:BankCode"];

            string createDate = order.purchaseDate != null ? 
                order.purchaseDate.ToString("yyyyMMddHHmmss") : 
                DateTime.Now.ToString("yyyyMMddHHmmss");

            string currCode = config["VnPay:CurrCode"];
            string locale = config["VnPay:Locale"];
            string orderInfo = order.Address.ToString();
            string orderType = order.orderStatus.ToString();
